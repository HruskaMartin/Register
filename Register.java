...
public class Register {
    ...
    public void addPerson(Person person) {
        ...
        for(int i = 0; i < count; i++) {
        	if(persons[i].getName().equals(person.getName())) {
        		if(persons[i].getPhoneNumber().equals(person.getPhoneNumber())) {
        			System.out.println("Takato osoba sa uz nachadza v registri");
        			return;
        		} else {
        ...
        System.out.println("Osoba bola pridana");
    ...
    public Person findPersonByName(String name) {
        for(int i = 0; i < count; i++) {
        	if(persons[i].getName().equals(name)) {
        		System.out.println("Je v databaze");
        	}else {}
        } 
        System.out.println("Neni v databaze");
		return null;     
    }
    ...
    public Person findPersonByPhoneNumber(String phoneNumber) {
    	for(int i = 0; i < count; i++) {
        	if(persons[i].getName().equals(phoneNumber)) {
        		System.out.println("Je v databaze");
        	}else {}
        }
		System.out.println("Neni v databaze");
    ...
    public void removePerson(Person person) {
     for (int i = 0; i < count; i++) {
    	 if(person.equals(persons[i])) {
    		 System.arraycopy(persons, i+1, persons, i, persons.length - 1 - i);
    		 count --;
